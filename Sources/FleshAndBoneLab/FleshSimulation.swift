...
final class FleshSimulation {
    ...
    init(device: MTLDevice, library: MTLLibrary, body: RuntimeBody,
        ...
        let vectorBytes = body.cellCount * MemoryLayout<SIMD4<Float>>.stride
    ...
    private func dispatch(_ encoder: MTLComputeCommandEncoder,
        ...
        encoder.dispatchThreads(
            MTLSize(width: body.cellCount, height: 1, depth: 1),
    ...
    func encodeFrame(commandBuffer: MTLCommandBuffer, substeps: Int = 4) {
        ...
        if let encoder = commandBuffer.makeComputeCommandEncoder() {
            ...
            encoder.setBuffer(body.skinIndices, offset: 0, index: 1)
            encoder.setBuffer(body.skinWeights, offset: 0, index: 2)
            encoder.setBuffer(body.skinMatrices, offset: 0, index: 3)
            encoder.setBuffer(lbsPrevious, offset: 0, index: 4)
            encoder.setBuffer(lbsCurrent, offset: 0, index: 5)
            encoder.setBuffer(lbsNext, offset: 0, index: 6)
            encoder.setBytes(
                &values, length: MemoryLayout<SimulationUniforms>.stride, index: 7)
        ...
        if physicsEnabled {
            for _ in 0..<max(substeps, 0) {
                if let observer = commandBuffer.makeComputeCommandEncoder() {
                    ...
                    observer.setBuffer(neighborResidual, offset: 0, index: 5)
                    observer.setBuffer(neighborVelocity, offset: 0, index: 6)
                    observer.setBuffer(compressionVector, offset: 0, index: 7)
                    observer.setBuffer(stretchVector, offset: 0, index: 8)
                    observer.setBuffer(densityScalars, offset: 0, index: 9)
                    observer.setBytes(
                        &values,
                ...
                if let integrator = commandBuffer.makeComputeCommandEncoder() {
                    ...
                    integrator.setBuffer(model.values, offset: 0, index: 11)
                    integrator.setBuffer(nextResidual, offset: 0, index: 12)
                    integrator.setBuffer(nextVelocity, offset: 0, index: 13)
                    integrator.setBytes(
                        &values,
                        length: MemoryLayout<SimulationUniforms>.stride,
                        index: 14)
    ...
    func resetDynamics() {
        phase = 0
        let bytes = body.cellCount * MemoryLayout<SIMD4<Float>>.stride
    ...
    func stateSummary() -> StateSummary {
        let values = residual.contents().bindMemory(
            to: SIMD4<Float>.self, capacity: body.cellCount)
        var squared: Double = 0
        var maximum: Float = 0
        var finiteCount = 0
        var nonfiniteCount = 0
        var firstNonfiniteIndex: Int?
        for index in 0..<body.cellCount {
    ...
    func diagnosticState(cell: Int) -> String {
        guard (0..<body.cellCount).contains(cell) else { return "cell out of range" }
        func vector(_ buffer: MTLBuffer) -> SIMD4<Float> {
            buffer.contents().bindMemory(
                to: SIMD4<Float>.self, capacity: body.cellCount)[cell]
        }
